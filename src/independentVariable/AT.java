...
public class AT {
    ...
    public void adaptiveTesting(){
        ...
        for (int y = 0; y < distribution.length; y++) {//对不同的变异体集进行测试
            for (int i = 0; i < numOfpartition.length; i++) {
                                    ...
                                    try{
                                        for (int n = 0; n < mutants[y].size(); n++) {
                                            ...
                                            for (int o = 0; o < methodsList.size(); o++) {
                                                //获取源程序的方法
                                                Method originalMethod = originalClazz.getMethod(methodsList.get(o),char.class,int.class,double.class,int.class,int.class);
                                                ...
                                                Method mutantMethod = mutantClazz.getMethod(methodsList.get(o),char.class,int.class,double.class,int.class,int.class);
                ...
                atLog.recordResult("ATResult_"+ String.valueOf(numOfpartition[i]) + ".txt",drtMeasure.getMeanFmeasure(),drtMeasure.getMeanNFmeasure(),
                        drtMeasure.getMeanTmeasure(),drtMeasure.getStandardDevOfFmeasure(),drtMeasure.getStandardDevOfNFmeasure(),
                        drtMeasure.getStandardDevOfTmeasure(),numOfpartition[i], meanfTime,meanf2Time,meantime);
            }
        }
    }
    ...
    private static final int NUMOFMUTANTS = 11 ;
